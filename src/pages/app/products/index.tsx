...
export function Products() {
  ...
  const { signed, user, addProductsToCart, addProduct } = useStore();
  ...
  const filteredProducts = useMemo(() => {
    const result = products?.filter(
      (product) =>
        ...
        (product.availabilityStatus === 'AVAILABLE' || role === 'ADMIN'),
    );
  ...
  function handleAddToCart() {
    const cupcakesToAdd = products
      ?.filter((cupcake) => (quantities[Number(cupcake.id)] || 0) > 0)
      .map((cupcake) => ({
        order_item_id: parseInt(uuidv4(), 16),
        order_id: parseInt(uuidv4(), 16),
        product_id: Number(cupcake.id),
        quantity: quantities[Number(cupcake.id)],
        created_at: new Date(),
      }));

    if (cupcakesToAdd && cupcakesToAdd.length > 0) {
      addProductsToCart(cupcakesToAdd, user?.mail ?? '');
                  ...
                  <div className="w-full md:w-4/5">
                    {filteredProducts?.map((product) => (
                      <ProductDetails
                        ...
                        quantityInput={
                          <QuantityInput
                            quantity={quantities[Number(product.id)] || 0}
                            onIncrease={() =>
                          ...
                          <TooltipTrigger asChild>
                            <Button
                              ...
                              disabled={
                                products?.filter(
                                  (cupcake) =>
                                    (quantities[Number(cupcake.id)] || 0) > 0,
                                ).length === 0
                              }
